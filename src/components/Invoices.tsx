      ...
      <header className="flex items-center justify-between mb-8 lg:mb-16">
        ...
        <div className="flex items-center">
          <div className="relative">
            ...
            >
              <span className="font-bold text-text-main mr-3 text-[13px] lg:text-sm lg:group-hover:text-text-accent transition-colors">Filter</span>
          ...
          <button 
            onClick={onOpenForm}
            className="flex items-center bg-button rounded-full p-1.5 lg:p-2 focus:outline-none cursor-pointer lg:hover:bg-button-hover transition-colors"
          >
        ...
        <section className="space-y-4">
          {filteredInvoices.map((invoice) => (
            <div 
              ...
              className="bg-container rounded-lg p-6 shadow-[0_10px_10px_-10px_rgba(72,84,159,0.1)] border border-transparent lg:hover:border-button transition-all cursor-pointer md:flex md:items-center md:justify-between md:px-8"
            >
              <div className="flex justify-between items-center mb-6 md:mb-0 md:gap-10">
                <span className="font-bold text-text-main text-[12px] md:w-20 transition-colors">
                  <span className="text-text-accent">#</span>{invoice.id}
                </span>
                <span className="text-text-secondary dark:text-text-main text-[12px] font-medium md:hidden transition-colors">{invoice.clientName}</span>
                <span className="text-text-secondary text-[12px] font-medium hidden md:block md:w-32 transition-colors">Due {invoice.paymentDue}</span>
                <span className="text-text-secondary dark:text-text-main text-[12px] font-medium hidden md:block md:w-32 transition-colors">{invoice.clientName}</span>
              </div>
              
              <div className="flex justify-between items-center md:flex-1 md:justify-end md:gap-10">
                <div className="flex flex-col">
                  <span className="text-text-secondary text-[12px] font-medium md:hidden mb-2 transition-colors">Due {invoice.paymentDue}</span>
                  <span className="text-text-main text-base font-bold md:w-32 md:text-right transition-colors">£ {invoice.total.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
